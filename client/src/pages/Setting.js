...
export default function Setting() {
  ...
  const changePassword = async (oldPassword, newPassword) => {
    ...
    await axios
      .put("http://localhost:5001/auth/ChangePassword/" + uID, updatePassword)
      .then((res) => {
        console.log(res);
        alert(res.status);
        if (res.status === 200) {
  ...
  const updateIncome = async (income) => {
    ...
    await axios
      .put("http://localhost:5001/income/" + uID, newIncome)
      .then((res) => {
        console.log(res);
        alert(res.status);
        if (res.status === 200) {
  ...
  const removeIncome = async () => {
    var uID = ls.get("userID");
    await axios
      .delete("http://localhost:5001/remove/income/" + uID)
      .then((res) => {
        console.log(res);
        if (res.status === 200) {
          removeCategories();
  ...
  const removeCategories = async () => {
    var uID = ls.get("userID");
    await axios
      .delete("http://localhost:5001/remove/categories/" + uID)
      .then((res) => {
        console.log(res);
        if (res.status === 200) {
        ...
        <Grid item xs={10}>
          <Container
            style={{
              ...
              background: "#1369fb",
              borderRadius: "8px",
              boxShadow: "2px 2px 2px 3px #9E9E9E",
            }}
          >
            ...
            >
              ...
              >
                <Grid item xs={8} sx={{ mt: 5 }}>
                  <p>
                    By clicking the reset button all the data stored in app will
                    be removed.{" "}
                  </p>
                </Grid>
                <Grid item xs={5}>
            ...
            >
              ...
              >
                <Grid item xs={10} sx={{ mt: 5, mb: 5 }}>
                  ...
                  Aliquam euismod ligula sapien, sagittis convallis lacus
                  vestibulum et. In hac habitasse platea dictumst. Nullam eget
                  arcu id diam viverra dignissim. Pellentesque dignissim quis
                  nisl sollicitudin consequat. Maecenas pulvinar lacus eu felis
                  tristique, hendrerit gravida arcu auctor. Donec lobortis velit
                  eu velit vestibulum, sit amet consectetur tellus ullamcorper.
                  Pellentesque sodales tortor eu sem cursus euismod.
                </Grid>
              </Grid>
            </Box>
          ) : null}
