...
function DashboardContent() {
  ...
  const addIncome = async (amount) => {
    ...
    await axios
      .post("http://localhost:5001/add/income", addIncome)
      .then((res1) => {
        console.log(res1);
        getTotalIncome();
      })
  ...
  const getTotalExpense = async () => {
    await axios
      .get("http://localhost:5001/total/expense/" + ls.get("userID"))
      .then((res) => {
  ...
  const getMonthlyExpenses = async () => {
    await axios
      .get("http://localhost:5001/monthly/expenses/" + ls.get("userID"))
      .then((res) => {
  ...
  const getTotalIncome = async () => {
    await axios
      .get("http://localhost:5001/total/income/" + ls.get("userID"))
      .then((res) => {
        if (res.data !== null) {
  ...
  const getTopCategories = async () => {
    await axios
      .get("http://localhost:5001/top/categories/" + ls.get("userID"))
      .then((res) => {
  ...
  return (
    <ThemeProvider theme={mdTheme}>
                ...
                {topCategories &&
                  topCategories.map((item) => {
                    return CategoriesList.map((category) => {
                      return category.title === item._id ? (
                        <Grid item xs={12} md={3}>
                          <Categories
                            title={item._id}
                            expense={item.sum}
      ...
      <Popup openPopup={open} setOpenPopup={setOpen} title={"Add Income"}>
          ...
          >
            <IconWrapperStyle>
              ...
              <img
                ...
                src="https://img.icons8.com/material-outlined/24/000000/low-price.png"
                alt=""
              />
            </IconWrapperStyle>
            <h4
